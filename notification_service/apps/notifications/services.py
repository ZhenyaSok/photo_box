...
class NotificationService:
    ...
    def create_with_fallback(
        self, user_id, title, message, notification_type="INFO", methods=None
    ):
        if methods is None:
        ...
        for method in methods:
            OutboxMessage.objects.create(
                notification=notification, method=method, status="PENDING"
            )
    ...
    def _try_send(self, outbox_msg, max_retries=3):
        ...
        payload = self._build_payload(
            outbox_msg.method, outbox_msg.notification, user_data
        )

        for attempt in range(max_retries):
            success = DeliveryService().send_via_method(
                outbox_msg.method, outbox_msg.notification, payload
            )
